	...
	public boolean mouseDown(double x, double y, AffineTransform myTransform) {
		for(int i = 0; i < contents.size(); i++){
			...
			if(content instanceof Selectable){
				...
				// forward each of the mouse events to the objects in "contents" in front to back order.
				boolean selected = shape.mouseDown(x, y, atf);
				// If any of them return true, stop forwarding the event to the remaining children and return true
				if(selected){
					return true;
				}
			}
	...
	public boolean mouseMove(double x, double y, AffineTransform myTransform) {
		for(int i = 0; i < contents.size(); i++){
			...
			if(content instanceof Selectable){
				...
				Interactable shape = (Interactable)content;
				// forward each of the mouse events to the objects in "contents" in front to back order.
				boolean selected = shape.mouseMove(x, y, atf);
				// If any of them return true, stop forwarding the event to the remaining children and return true
				if(selected){
					return true;
				}
	...
	public boolean mouseUp(double x, double y, AffineTransform myTransform) {
		for(int i = 0; i < contents.size(); i++){
			...
			if(content instanceof Selectable){
				...
				Interactable shape = (Interactable)content;
				// forward each of the mouse events to the objects in "contents" in front to back order.
				boolean selected = shape.mouseUp(x, y, atf);
				// If any of them return true, stop forwarding the event to the remaining children and return true
				if(selected){
					return true;
				}
